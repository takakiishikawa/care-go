...
import { Wind, CheckCircle, LayoutDashboard, Brain, Activity, Loader2 } from 'lucide-react';
import CareComment from '@/components/ui/CareComment';
import { Button, Card, CardHeader, CardContent, CardDescription } from '@takaki/go-design-system';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-foreground tracking-tight">記録しました</h1>
        ...
        <div className="grid grid-cols-3 gap-3">
          {([
            ...
            >
              ...
              <p className="text-[28px] font-extrabold leading-none tabular-nums" style={{ color, letterSpacing: '-0.04em' }}>
                {value || '–'}
              </p>
            </div>
          ))}
      ...
      <Card
        className="p-5"
        style={{ background: 'var(--color-warning-subtle)', borderColor: 'var(--color-warning)' } as React.CSSProperties}
      >
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {isLogging ? <Loader2 size={13} className="animate-spin" /> : <Wind size={13} strokeWidth={2} />}
            {meditationLogged ? '記録済み' : '瞑想に進む'}
          </Button>
          <p className="text-xs text-muted-foreground">別タブでYouTubeが開きます</p>
        </div>
      </Card>
